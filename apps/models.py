...
class ResNet9(ndl.nn.Module):
    ...
    def ConvBN(self, in_channel, out_channel, kernel_size, stride):
        sequential = nn.Sequential(
            nn.Conv(in_channel, out_channel, kernel_size = kernel_size, stride = stride, device = self.device, dtype = self.dtype),
            nn.BatchNorm2d(out_channel, device = self.device, dtype = self.dtype),
            nn.ReLU()
        )
        return sequential
